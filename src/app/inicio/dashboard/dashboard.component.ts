...
export class DashboardComponent implements OnInit, OnDestroy {
  ...
  getFirstPhoto(property: Propiedad): string {
    if (property.imagen_principal) return getImageUrl(property.imagen_principal);
    if (property.fotos && Array.isArray(property.fotos) && property.fotos.length > 0) return getImageUrl(property.fotos[0]);
