...
@Component({
    ...
    styleUrls: ["./editarPerfil.component.scss"], // Cambiado de .css a .scss
})
export class EditarPerfilComponent implements OnInit {
    ...
    isSaving: boolean = false; // Controla el estado de guardado (para el botón de enviar)
    ...
    formChanges: any = {}; // Objeto para rastrear los cambios en el formulario

    // PROPIEDADES AÑADIDAS PARA LA GESTIÓN DE IMAGENES
    selectedImage: string | ArrayBuffer | null = null; // Para previsualizar la imagen seleccionada
     ...
     * Útil si el usuario decide no subir una imagen seleccionada.
     */
    ...
    uploadProfilePicture(): void {
        ...
        formData.append('profile_picture', this.selectedFile); // Añadir el archivo con la clave esperada
        ...
        this.Shttp.Service_Post('update-profile-picture', formData).subscribe({
            next: (response: any) => {
                ...
                // Si el backend devuelve la nueva URL, actualizarla. Añadir un parámetro de timestamp para evitar caché.
                if (this.userData && response.profile_picture_url) {
                    this.userData.profile_picture = `${response.profile_picture_url}?${new Date().getTime()}`;
                    window.location.reload(); // Recargar la página para asegurar que la nueva imagen se muestre
                } else {
            ...
            error: (err) => {
                ...
                // Extraer mensaje de error de la respuesta del backend o usar uno genérico
...
<<<<<<< HEAD
        ...
        this.Shttp.Service_Put(`users/${this.userData.id}`, updateData).subscribe({
            ...
            error: (err) => {
                ...
                // Formatear mensajes de error para una mejor visualización
